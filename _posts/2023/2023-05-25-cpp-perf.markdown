...
your timer will report about **100 ms**, even though CPU time is almost zero. For actual CPU time, you would use things like: `std::clock()`

- `std::chrono::steady_clock` : monotonically increasing clock, doesn't change if system clock changes
- `std::chrono::high_resolution_clock`: walltime
- `clock_gettime(CLOCK_MONOTONIC, ...)`: wall time
- `clock_gettime(CLOCK_PROCESS_CPUTIME_ID, ...)` cpu timee of the whole process

#### Is CPU time always shorter than wall time?

No. For a **single-threaded** program, CPU time is usually: `CPU time <= wall time`, But for a **multi-threaded** program, process CPU time can be **greater** than wall time, because CPU time is summed across cores.
