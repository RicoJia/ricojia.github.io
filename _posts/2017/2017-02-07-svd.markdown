...
A perfect rotation matrix R has

$$
R = U I Vᵀ
$$

When you multiply two matrices together, though they are SE3 matrices, there still could be numerical errors. When that happens, R can be adjusted to $U V^T$ because without numerical errors, those should be R's values

```cpp
Eigen::Isometry3d guess = last;
Sophus::SE3d full_delta  = Sophus::SE3d::exp(xi);

guess.matrix() = last.matrix() * full_delta.matrix();

// Reorthogonalize rotation to avoid numerical drift from matrix composition
Eigen::Matrix3d R = guess.linear();
Eigen::JacobiSVD<Eigen::Matrix3d> svd(R, Eigen::ComputeFullU | Eigen::ComputeFullV);
guess.linear() = svd.matrixU() * svd.matrixV().transpose();
```
...
## Gotcha - SVD Could Create a Reflection When Solving For a Rigid Transform

Find R that aligns source points P (3xn) to target poin ts Q (3xn). Usually, we put [x,y,z; x2, y2, z2] *[qx, qy, qz; ...] = sum(q_i* p_i ^T) so it's still 3x3 = Q * P^T

Then SVD: H = U Σ V^T. Unconstrained optimal orthogonal transform is R = U V^T, but R is only guaranteed to be normal: R^T R = I. In the mean time, a pure rotation must satisfy det(R) = 1, but reflection has det(R) = -1 . This is because det(R) = det(U V^T) = det(U) det (V^T). either determinant could be 1 or -1.
...
A reflection matrix has det(R) = -1, like U = [-1 0 0 1]. above, `det(U Vᵀ) = -1`

Or, think about how SVD solved for U and V. U columns are eigen vectors of H HT.
...
if we flip a column (which is still a valid eigen vector), we get a flipped determinant
...
To fix, you flip the last column and try again:
